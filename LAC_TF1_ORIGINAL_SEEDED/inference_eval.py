...
if __name__ == "__main__":
    ...
    for name in eval_agents:
        ...
        if args.plot_r:
            ...
            for i in range(0, max(soi_mean_path.shape[0], ref_mean_path.shape[0])):
                if (i + 1) in req_ref or not req_ref:
                    fig = plt.figure(
                        figsize=(9, 6), num=f"LAC_TF1_ORIGINAL_SEEDED_{i+1}"
                    )
        ...
        if args.plot_o:
            ...
            fig = plt.figure(figsize=(9, 6), num=f"LAC_TF1_ORIGINAL_SEEDED_{i+2}")
            colors = "bgrcmk"
            cycol = cycle(colors)
            obs_trimmed = [
                path
