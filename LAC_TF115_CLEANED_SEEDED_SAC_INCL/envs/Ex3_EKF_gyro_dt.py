...
def omega_t_sim3(t):
    ...
    if (0.5 * math.pi / 0.008) < t <= (1.5 * math.pi / 0.008):
        omega_1 = (
            np.sin((0.008 * t) - 0.5 * math.pi) * 0.4
        )  # the real angular velocity, in q_pred direction
        omega_2 = 0  # the real angular velocity, in y direction
        omega_3 = 0  # the real angular velocity, in z direction
    else:
...
def diata_q_dot(t, diata_q, omege_noise_t):
    ...
    M = quatRightMulMat(np.hstack(quatPure2Q(np.hstack(omega_t)))) - quatLeftMulMat(
        np.hstack(quatPure2Q(np.hstack(omega_obs)))
    )
...
class Ex3_EKF_gyro(gym.Env):
    ...
    def step(self, action):  # here u1,u2=measurement, which is a result of the action
        ...
        if not self.clean:
            ...
            noise_i = (
                np.random.multivariate_normal([0, 0, 0], self.cov_noise_i).flatten() * 0
            )
        ...
        if not self.clean:
            acc_i = np.random.multivariate_normal([0.0, 0.0, 0.0], self.cov_a).flatten()
        else:
            acc_i = (
                np.random.multivariate_normal([0.0, 0.0, 0.0], self.cov_a).flatten() * 0
            )
        ...
        aaa = 2.0 * Log(
            np.inner(quatRightMulMat(quatConj(self.q_pred_init)), self.q_t_init)
        )
        # bbb = 2 / self.dt * Log(np.inner(quatLeftMulMat(q_t_lastStep),np.hstack(quatConj(np.inner(quatLeftMulMat(q_pred),hat_delta_q))))) - np.hstack(omega)
        bbb = 2 / self.dt * Log(
            np.inner(quatLeftMulMat(quatConj(q_t_lastStep)), np.hstack((hat_q)))
        ) - np.hstack(omega)
        ccc = y - hat_y
        cost = (
            np.linalg.norm(aaa) ** 2
            + np.linalg.norm(bbb) ** 2
            + np.linalg.norm(ccc) ** 2
        )
