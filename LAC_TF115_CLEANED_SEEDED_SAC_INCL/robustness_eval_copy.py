...
def get_distrubance_function(env_name):
    ...
    elif 'Ex1_KF' in env_name:
        disturbance_step = Ex1_KF_disturber
    elif 'Ex2_KF' in env_name:
        disturbance_step = Ex2_KF_disturber
    elif 'Ex2_EKF' in env_name:
        disturbance_step = Ex2_EKF_disturber
    elif 'Ex3_EKF' in env_name:
        disturbance_step = Ex3_EKF_disturber
    elif 'Ex4_EKF' in env_name:
        disturbance_step = Ex4_EKF_disturber
    elif 'pose_Est' in env_name:
        disturbance_step = pose_Est_disturber
    elif 'spacecraft' in env_name:
        disturbance_step = spacecraft_disturber
    elif 'spacecraft_e' in env_name:
        disturbance_step = spacecraft_disturber
    elif 'spacecraft_f' in env_name:
        disturbance_step = spacecraft_disturber
...
def Ex1_KF_disturber(time, s, action, env, eval_params, form_of_eval, disturber=None):
    if form_of_eval == 'impulse':
        if time ==eval_params['impulse_instant']:
...
def Ex2_KF_disturber(time, s, action, env, eval_params, form_of_eval, disturber=None):
    if form_of_eval == 'impulse':
        if time ==eval_params['impulse_instant']:
...
def Ex2_EKF_disturber(time, s, action, env, eval_params, form_of_eval, disturber=None):
    if form_of_eval == 'impulse':
        if time ==eval_params['impulse_instant']:
...
def Ex3_EKF_disturber(time, s, action, env, eval_params, form_of_eval, disturber=None):
    if form_of_eval == 'impulse':
        if time ==eval_params['impulse_instant']:
            d = eval_params['magnitude'] * (-np.sign(action))
        else:
...
def Ex4_EKF_disturber(time, s, action, env, eval_params, form_of_eval, disturber=None):
    if form_of_eval == 'impulse':
        if time ==eval_params['impulse_instant']:
...
def pose_Est_disturber(time, s, action, env, eval_params, form_of_eval, disturber=None):
    if form_of_eval == 'impulse':
        if time ==eval_params['impulse_instant']:
...
def spacecraft_disturber(time, s, action, env, eval_params, form_of_eval, disturber=None):
    if form_of_eval == 'impulse':
        if time ==eval_params['impulse_instant']:
...
def dynamic(variant):
    ...
    np.savetxt(str(i)+'.csv', path_set, delimiter = ',')
    ...
    dt = 0.1
    t = dt*np.array(range(max_len))
    ax.plot(t, average_path[:,0], color='red',label = 'cost')
    for path in paths['c']:
        ax.plot(t, path, color='red',label = 'cost', linestyle='--')
    ax.legend()
    plt.xlabel('time (sec)')
    plt.ylabel(r'$\theta$ (rad); $\omega$ (rad/sec)')
    plt.show()
    ############
    ...
    if eval_params['directly_show']:      
        ...
        fig = plt.figure(figsize=(9, 6))
        ax = fig.add_subplot(111)
        ax.plot(t, average_path[:,8], color='red',label = r'$eta_q_0$', linestyle='--')
        ax.plot(t, average_path[:,9], color='blue', label = r'$eta_q_1$', linestyle='--')
        ax.plot(t, average_path[:,10], color='yellow',label = r'$eta_q_2$', linestyle='--') 
        plt.show()
        
        fig = plt.figure(figsize=(9, 6))
        ax = fig.add_subplot(111)
        ax.plot(t, average_path[:,11], color='red',label = r'$cost$', linestyle='--')
        plt.show()
        ...
        if eval_params['plot_average']:
            ax.plot(t, average_path[:,0], color='red',label = r'$hat_q_0$', linestyle='--')
            ax.plot(t, average_path[:,1], color='blue', label = r'$hat_q_1$', linestyle='--')
            ax.plot(t, average_path[:,2], color='yellow',label = r'$hat_q_2$', linestyle='--')
            ax.plot(t, average_path[:,3], color='green', label = r'$hat_q_3$', linestyle='--')
            ax.plot(t, average_path[:,4], color='red',label = r'$q_t_0$')
            ax.plot(t, average_path[:,5], color='blue', label = r'$q_t_1$')
            ax.plot(t, average_path[:,6], color='yellow',label = r'$q_t_2$')
            ax.plot(t, average_path[:,7], color='green', label = r'$q_t_3$')

            ax.fill_between(t, average_path[:,0] - std_path[:,0], average_path[:,0] + std_path[:,0] , color='red', alpha=.1)
            ax.fill_between(t, average_path[:,1] - std_path[:,1], average_path[:,1] + std_path[:,1], color='blue', alpha=.1)
            ax.fill_between(t, average_path[:,2] - std_path[:,2], average_path[:,2] + std_path[:,2] , color='yellow', alpha=.1)
            ax.fill_between(t, average_path[:,3] - std_path[:,3], average_path[:,3] + std_path[:,3], color='green', alpha=.1)
            ...
            # plt.savefig(variant['eps_Name'],format="eps")
            sv_path = variant['log_path']
            trajNum = variant['trajNum']
            plt.savefig(f'{sv_path}/trajectory{trajNum}.pdf')           
        else:
            for path in paths['s']:
                path_length = len(path)
...
def evaluation(variant, env, policy, disturber= None):
    ...
    for trial in trial_list:
        ...
        print('seed_average_cost', seed_average_cost[0])
