    ...
    for name in eval_agents:
        ...
        if args.plot_r:
            ...
            if EVAL_PARAMS["merged"]:
                fig_1 = plt.figure(
                    figsize=(9, 6), num=f"LAC_TF2_CLEANED_EAGER_SPEEDUP_1"
                )
            ...
            for i in range(0, max(soi_mean_path.shape[0], ref_mean_path.shape[0])):
                if (i + 1) in req_ref or not req_ref:
                    if not EVAL_PARAMS["merged"]:
                        fig_1 = plt.figure(
                            figsize=(9, 6), num=f"LAC_TF2_CLEANED_EAGER_SPEEDUP_{i+1}",
                        )
                        ax = fig_1.add_subplot(111)
                        color1 = "red"
                        color2 = "blue"
        ...
        if args.plot_o:
            ...
            fig_2 = plt.figure(figsize=(9, 6), num="LAC_TF2_CLEANED_EAGER_SPEEDUP_2")
            colors = "bgrcmk"
            cycol = cycle(colors)
