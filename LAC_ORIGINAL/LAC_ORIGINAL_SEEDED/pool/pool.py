    ...
    def store(self, s, a, d, raw_d, r, terminal, s_):
        ...
        if terminal == 1.0:
            if "value" in self.memory.keys():
                ...
                r_N_ = []
                [
                    value.append(r[i : i + self.horizon, 0].sum())
                    for i in range(path_length)
                ]
                [r_N_.append(r[i + self.horizon + 1, 0]) for i in range(path_length)]
                value = np.array(value)
                r_N_ = np.array(r_N_)
                self.memory["value"] = np.concatenate(
                    (self.memory["value"], value[:, np.newaxis]), axis=0
                )
                self.memory["r_N_"] = np.concatenate(
                    (self.memory["r_N_"], r_N_[:, np.newaxis]), axis=0
                )
    ...
    def sample(self, batch_size):
        if self.memory_pointer < self.min_memory_size:
            ...
            indices = np.random.choice(
                min(self.memory_pointer, self.memory_capacity)
                - 1
                - self.history_horizon,
                size=batch_size,
                replace=False,
            ) + max(
                1 + self.history_horizon,
                1 + self.history_horizon + self.memory_pointer - self.memory_capacity,
            ) * np.ones(
                [batch_size], np.int
            )
            batch = {}
            ...
            for key in self.memory.keys():
                if "s" in key:
                    sample = [
                        self.memory[key][indices - i]
                        for i in range(self.history_horizon + 1)
                    ]
