...
class CarEnv(object):
    ...
    def __init__(self, discrete_action=False):
        ...
        self.terminal = False
        # node1 (x, y, r, w, l),
        self.car_info = np.array(
            [0, 0, 0, 20, 40], dtype=np.float64
        )  # car coordination
        self.obstacle_coords = np.array(
            [[120, 120], [380, 120], [380, 380], [120, 380],]
        )
        self.sensor_info = self.sensor_max + np.zeros(
            (self.n_sensor, 3)
        )  # n sensors, (distance, end_x, end_y)

        self.action_space = spaces.Box(low=-1, high=1, shape=(1,), dtype=np.float32)
        self.observation_space = spaces.Box(
            -self.sensor_max * np.ones([self.state_dim]),
            self.sensor_max * np.ones([self.state_dim]),
            dtype=np.float32,
        )
    ...
    def step(self, action):
        ...
        self.car_info[:2] = self.car_info[:2] + self.speed * self.dt * np.array(
            [np.cos(self.car_info[2]), np.sin(self.car_info[2])]
        )

        self._update_sensor()
        s = self._get_state()
        r = 1000 if self.terminal else abs(action)
        return s, r, self.terminal, dict(l_rewards=0, violation_of_constraint=0)
    ...
    def render(self):
        if self.viewer is None:
            self.viewer = Viewer(
                *self.viewer_xy, self.car_info, self.sensor_info, self.obstacle_coords
            )
        self.viewer.render()
    ...
    def _update_sensor(self):
        ...
        for si in range(len(self.sensor_info)):
            ...
            for oi in range(len(self.obstacle_coords)):
                ...
                r = (
                    self.obstacle_coords[(oi + 1) % len(self.obstacle_coords)]
                    - self.obstacle_coords[oi]
                )
            ...
            for oi in range(4):
                ...
                if np.cross(r, s) != 0:  # may collision
                    ...
                    if 0 <= t <= 1 and 0 <= u <= 1:
                        intersection = p + t * r
                        possible_intersections.append(intersection)
                        possible_sensor_distance.append(
                            np.linalg.norm(intersection - q)
                        )
...
class Viewer(pyglet.window.Window):
    ...
    def __init__(self, width, height, car_info, sensor_info, obstacle_coords):
        super(Viewer, self).__init__(
            width, height, resizable=False, caption="2D car", vsync=False
        ...
        for i in range(len(self.sensor_info)):
            self.sensors.append(
                self.batch.add(
                    2, pyglet.gl.GL_LINES, foreground, ("v2f", line_coord), ("c3B", c)
                )
            )
        ...
        self.car = self.batch.add(
            4, pyglet.gl.GL_QUADS, foreground, ("v2f", car_box), ("c3B", c)
        )
