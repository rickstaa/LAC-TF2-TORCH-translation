                ...
                for alg_dir in alg_dirs:
                    ...
                    if alg_dir in alg_list:
                        ...
                        if args["data"] == "training":
                            ...
                            result = read_eval_data(
                                alg_path, args["eval_content"], content
                            )
    ...
    for content in contents:
        ...
        for exp in results.keys():
            ...
            if args["formal_plot"]:
                plt.xlim(
                    0,
                    round(result["total_timesteps"].values[min_length - 1] / 1000, 1)
                    * 1000,
                )
            ...
            for alg_index, alg in enumerate(algs):
                ...
                try:
                    img = ax.pcolormesh(
                        result["x"],
                        result["y"],
                        result["measure"][content],
                        cmap=cm,
                        vmin=result["lower_bound"][content],
                        vmax=result["upper_bound"][content],
                        edgecolors="face",
                    )
                except TypeError:
                ...
                cbar.set_ticks(
                    np.linspace(
                        result["lower_bound"][content],
                        result["upper_bound"][content],
                        5,
                    )
                )
...
def plot_line(results, alg_list, eval_contents, measure_list, exp, figsize=None):
    ...
    for eval_index, eval_name in enumerate(eval_contents):
        ...
        for content_index, content in enumerate(measure_list):
            ...
            ax.legend(
                handles, labels, fontsize=font, loc=2, fancybox=False, shadow=False
            )
            ax.grid(True)
            plt.xticks(fontsize=tick_fontsize)
            plt.yticks(fontsize=tick_fontsize)
