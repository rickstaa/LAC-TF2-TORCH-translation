...
def halfcheetah_disturber(
    time, s, action, env, eval_params, form_of_eval, disturber=None
):
    if form_of_eval == "impulse":
        if time == eval_params["impulse_instant"]:
...
def param_variation(variant):
    ...
    policy_params.update(
        {"s_bound": env.observation_space, "a_bound": env.action_space,}
    )
...
def dynamic(variant):
    ...
    log_path = (
        variant["log_path"] + "/eval/dynamic/" + eval_params["additional_description"]
    )
    variant["eval_params"].update({"magnitude": 0})
    logger.configure(dir=log_path, format_strs=["csv"])
...
def evaluation(variant, env, policy, disturber=None):
    ...
    for trial in trial_list:
        ...
        success_load = policy.restore(
            os.path.join(variant["log_path"], trial) + "/policy"
        )
        if not success_load:
            continue
        die_count = 0
        ...
        for i in range(
            ...
            for j in range(max_ep_steps):
                ...
                if form_of_eval == "trained_disturber":
                    s_, r, done, info = disturbance_step(
                        j,
                        s,
                        action,
                        env,
                    ...
                    s_, r, done, info = disturbance_step(
                        j, s, action, env, eval_params, form_of_eval
                    )
...
def training_evaluation(variant, env, policy, disturber=None):
    ...
    diagnostic = {"return": total_cost_mean, "average_length": average_length}
    return diagnostic
