...
class Disturber(object):
    def __init__(
        ...
        self.memory = np.zeros(
            (self.memory_capacity, s_dim * 2 + a_dim + 3), dtype=np.float32
        )
        self.pointer = 0
        ...
        with tf.variable_scope("disturber"):
            ...
            a_params = tf.get_collection(
                tf.GraphKeys.TRAINABLE_VARIABLES, scope="disturber/Actor"
            )
            c1_params = tf.get_collection(
                tf.GraphKeys.TRAINABLE_VARIABLES, scope="disturber/critic1"
            )
            c2_params = tf.get_collection(
                tf.GraphKeys.TRAINABLE_VARIABLES, scope="disturber/critic2"
            )
            ...
            q1_ = self._build_c(
                self.S_, a_, "critic1", reuse=True, custom_getter=ema_getter
            )
            q2_ = self._build_c(
                self.S_, a_, "critic2", reuse=True, custom_getter=ema_getter
            )
        ...
        self.a_loss = tf.reduce_mean(
            self.alpha * tf.expand_dims(log_pis, axis=1)
            - min_Q_target
            - policy_prior_log_probs
        )

        self.atrain = tf.train.AdamOptimizer(self.LR_A).minimize(
            self.a_loss, var_list=a_params
        )  # 以learning_rate去训练，方向是minimize loss，调整列表参数，用adam
        ...
        with tf.control_dependencies(
            ...
            q1_target = (
                self.R
                - ita * tf.expand_dims(tf.norm(self.real_a_input, axis=1), axis=1)
                + gamma * (1 - self.terminal) * tf.stop_gradient(min_next_q)
            )  # ddpg
            q2_target = (
                self.R
                - ita * tf.expand_dims(tf.norm(self.real_a_input, axis=1), axis=1)
    ...
    def choose_action(self, s, k, evaluation=False):
        if evaluation is True:
            ...
            if self.energy_bounded:
                a = -self.magnitude + (raw_a + 1.0) * self.magnitude
    ...
    def learn(self, LR_A, LR_C, batch):
        ...
        alpha, q1_error, q2_error, entropy, a_loss = self.sess.run(
            self.diagnotics, feed_dict
        )
    ...
    def _build_a(self, s, name="Actor", reuse=None, custom_getter=None):
        ...
        with tf.variable_scope(name, reuse=reuse, custom_getter=custom_getter):
            ...
            net_0 = tf.layers.dense(
                s, 64, activation=tf.nn.relu, name="l1", trainable=trainable
            )  # 原始是30
            net_1 = tf.layers.dense(
                net_0, 64, activation=tf.nn.relu, name="l4", trainable=trainable
            )  # 原始是30
            mu = tf.layers.dense(
            ...
            if False:
                ...
                bijector = tfp.bijectors.Chain(
                    (tfp.bijectors.Affine(shift=mu, scale_diag=sigma),)
                )
    ...
    def _build_c(self, s, a, name="Critic", reuse=None, custom_getter=None):
        ...
        with tf.variable_scope(name, reuse=reuse, custom_getter=custom_getter):
            ...
            net_1 = tf.layers.dense(
                net_0, 64, activation=tf.nn.relu, name="l2", trainable=trainable
            )  # 原始是30
