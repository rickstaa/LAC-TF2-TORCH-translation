...
class LAC(object):
    ...
    def __init__(self, a_dim, s_dim, log_dir="."):
        ...
        # self.polyak = 1 - ALG_PARAMS["tau"]  # DEBUG: IS this needed on the GPU
        self.polyak = torch.as_tensor((1 - ALG_PARAMS["tau"]), dtype=torch.float32)
        self.gamma = torch.as_tensor(ALG_PARAMS["gamma"], dtype=torch.float32)
        self.alpha_3 = torch.as_tensor(ALG_PARAMS["alpha3"], dtype=torch.float32)
        ...
        if ALG_PARAMS["target_entropy"] is None:
            ...
            self.target_entropy = torch.as_tensor(
                -self.a_dim, dtype=torch.float32
            )  # lower bound of the policy entropy
        else:
            # self.target_entropy = ALG_PARAMS["target_entropy"] # DEBUG
            self.target_entropy = torch.as_tensor(
                ALG_PARAMS["target_entropy"], dtype=torch.float32
            )
        ...
        self.log_labda = torch.tensor(ALG_PARAMS["labda"], dtype=torch.float32).log()
        ...
        if evaluation is True:
            try:
                with torch.no_grad():
                    ...
                    # return deterministic_a[0].numpy() # Debug
                    return deterministic_a[0].cpu().numpy()
            except ValueError:
                return
        else:
            with torch.no_grad():
                a, _, _ = self.ga(s.unsqueeze(0))
                # return a[0].numpy() # DEBUG
    ...
    def learn(self, LR_A, LR_L, LR_lag, batch):
        ...
        self.l_delta = torch.mean(l_ - l.detach() + self.alpha_3 * br)
        ...
        return (
            self.labda.detach(),
            self.alpha.detach(),
            l_error.detach(),  # FIXME: Not needed in spinning up since the logger can handle GPU data
            torch.mean(-log_pis.detach()),
            a_loss.detach(),
            # l_error.detach().cpu(),  # FIXME: Not needed in spinning up since the logger can handle GPU data
            # torch.mean(-log_pis.detach().cpu()),
            # a_loss.detach().cpu(),
        )
    ...
    for i in range(ENV_PARAMS["max_episodes"]):
        ...
        for j in range(ENV_PARAMS["max_ep_steps"]):
            ...
            if global_step > ENV_PARAMS["max_global_steps"]:
                ...
                print("Saving Model")
                policy.save_result(log_dir)  # DEBUG
                print("Running time: ", time.time() - t1)
                return
            ...
            if training_started:
                ...
                # current_path["a_loss"].append(a_loss)  #DEBUG
                    ...
                    if TRAIN_PARAMS["num_of_evaluation_paths"] > 0:
                        [
                            string_to_print.extend(
                                [key, ":", str(eval_diagnostics[key].numpy()), "|"]
                            )
                            for key in eval_diagnostics.keys()
                        ]
                    # [
