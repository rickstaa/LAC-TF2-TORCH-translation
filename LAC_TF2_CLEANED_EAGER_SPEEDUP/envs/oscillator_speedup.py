...
class oscillator(gym.Env):
    ...
    def step(self, action):
        ...
        # m1_tmp = m1 + m1_dot * self.dt + np.random.uniform(-self.sigma, self.sigma, 1)
        # m1 = m1_tmp[0] if m1_tmp >= 0.0 else 0.0
        # m2_tmp = m2 + m2_dot * self.dt + np.random.uniform(-self.sigma, self.sigma, 1)
        # m2 = m2_tmp[0] if m2_tmp>= 0.0 else 0.0
        # m3_tmp = m3 + m3_dot * self.dt + np.random.uniform(-self.sigma, self.sigma, 1)
        # m3 = m3_tmp[0] if m3_tmp >= 0.0 else 0.0
        ...
        # p1_tmp = p1 + p1_dot * self.dt + np.random.uniform(-self.sigma, self.sigma, 1)
        # p1 = p1_tmp[0] if p1_tmp >= 0.0 else 0.0
        # p2_tmp = p2 + p2_dot * self.dt + np.random.uniform(-self.sigma, self.sigma, 1)
        # p2 = p2_tmp[0] if p2_tmp >= 0.0 else 0.0
        # p3_tmp = p3 + p3_dot * self.dt + np.random.uniform(-self.sigma, self.sigma, 1)
        # p3 = p3_tmp[0] if p3_tmp >= 0.0 else 0.0
    ...
    def reset(self):
        ...
        # self.state = np.array([0.1,0.2,0.3,0.1,0.2,0.3])
        self.t = 0
        m1, m2, m3, p1, p2, p3 = self.state
        r1 = self.reference(self.t)
