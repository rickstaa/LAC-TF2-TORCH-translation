...
class Ex1_EKF(gym.Env):
    def __init__(self):
        ...
        self.cov1 = np.array(
            [
                [1 / 3 * (self.dt) ** 3 * self.q1, 1 / 2 * (self.dt) ** 2 * self.q1, 0],
                [1 / 2 * (self.dt) ** 2 * self.q1, self.dt * self.q1, 0],
                [0, 0, self.dt * self.q2],
            ]
        )
        self.mean2 = 0
    ...
    def step(self, action):
        ...
        x = np.dot(self.A, x) + np.random.multivariate_normal(
            self.mean1, self.cov1
        ).reshape((3, 1))
        # x = np.dot(self.A, x)
        ...
        return (
            np.array([hat_x[0, 0], hat_x[1, 0], hat_x[2, 0], y]),
            cost,
            done,
            dict(reference=y, state_of_interest=hat_x[0, 0] - x[0, 0]),
        )

    def reset(self):
        self.state = np.concatenate(
            (np.random.multivariate_normal(self.mean0, self.cov0), np.array(self.mean0))
        )
        hat_x_1, hat_x_2, hat_x_3, x_1, x_2, x_3 = self.state
